...
class GroupForm implements FormModel<Group, Group> {
  ...
  Personal? get _personalValue => containsPersonal ? personalForm.model : null;

  Phone? get _phoneValue => containsPhone ? phoneForm.model : null;

  Address? get _addressValue => containsAddress ? addressForm.model : null;

  Address? get _address2Value => containsAddress2 ? address2Form.model : null;

  Personal? get _personalRawValue =>
      containsPersonal ? personalForm.rawModel : null;

  Phone? get _phoneRawValue => containsPhone ? phoneForm.rawModel : null;

  Address? get _addressRawValue =>
      containsAddress ? addressForm.rawModel : null;

  Address? get _address2RawValue =>
  ...
  PersonalForm get personalForm =>
      PersonalForm(form, pathBuilder('personal'), _formModel ?? this);

  PhoneForm get phoneForm =>
      PhoneForm(form, pathBuilder('phone'), _formModel ?? this);

  AddressForm get addressForm =>
      AddressForm(form, pathBuilder('address'), _formModel ?? this);

  AddressForm get address2Form =>
      AddressForm(form, pathBuilder('address2'), _formModel ?? this);
...
class PersonalForm implements FormModel<Personal, Personal> {
  PersonalForm(this.form, this.path, this._formModel) : initial = form.rawValue;
...
class PhoneForm implements FormModel<Phone, Phone> {
  PhoneForm(this.form, this.path, this._formModel) : initial = form.rawValue;
