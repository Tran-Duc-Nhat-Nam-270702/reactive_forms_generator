...
class ReactiveTagsOFormConsumer<T> extends StatelessWidget {
  const ReactiveTagsOFormConsumer({Key? key, required this.builder, this.child})
    : super(key: key);
...
class _TagsOFormBuilderState<T> extends State<TagsOFormBuilder<T>> {
  ...
  @override
  void initState() {
    _formModel = TagsOForm<T>(
      TagsOForm.formElements<T>(widget.model),
      null,
      null,
    );
...
class TagsOForm<T> implements FormModel<TagsO<T>, TagsOOutput<T>> {
  TagsOForm(this.form, this.path, this._formModel) : initial = form.rawValue;
  ...
  void updateValue(
    ...
    TagsOForm.formElements(value).rawValue,
    updateParent: updateParent,
    emitEvent: emitEvent,
  );
  ...
  void upsertValue(
    ...
    final formElements = TagsOForm.formElements(value);
...
class ReactiveTagsOFormFormGroupArrayBuilder<
  ReactiveTagsOFormFormGroupArrayBuilderT,
  T
>
    extends StatelessWidget {
